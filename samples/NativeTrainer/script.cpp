...
void process_location_menu()
{
    char caption[32];
    Entity e = PLAYER::PLAYER_PED_ID();
    Vector3 location = ENTITY::GET_ENTITY_COORDS(e,true);
    float a = location.x;
    float b = location.y;
    float c = location.z;
    ...
    GRAPHICS::_WORLD3D_TO_SCREEN2D(a,b,c,x2d,y2d);
    
    std::string loc = "2Dcoord:(" + std::to_string(*x2d) +"," + std::to_string(*y2d) +")";
    //Player player = PLAYER::PLAYER_ID();
    //sprintf_s(caption, "Location   %d / %d", ENTITY::GET_ENTITY_COORDS(e, true));
    set_status_text(loc);
}
...
void process_cleararea_menu()
{
    ...
    std::string caption = "CLEAR  OPTIONS";
    ...
    while (true)
    {
        ...
        do
        {
            ...
            draw_menu_line(caption, lineWidth, 15.0, 18.0, 0.0, 5.0, false, true);
            for (int i = 0; i < lineCount; i++)
                if (i != activeLineIndexClear)
                    draw_menu_line(line_as_str(lines[i].text, lines[i].pState),
                                   lineWidth, 9.0, 60.0 + i * 36.0, 0.0, 9.0, false, false);
            draw_menu_line(line_as_str(lines[activeLineIndexClear].text, lines[activeLineIndexClear].pState),
                           lineWidth + 1.0, 11.0, 56.0 + activeLineIndexClear * 36.0, 0.0, 7.0, true, false);
        ...
        if (bSelect)
        {
            ...
            // common variable
            Entity e = PLAYER::PLAYER_PED_ID();
            Vector3 playerLoc = ENTITY::GET_ENTITY_COORDS(e,true);
            
            switch (activeLineIndexClear)
            {
                    
                case 0:
                {
                    GAMEPLAY::CLEAR_AREA_OF_PEDS(playerLoc.x,playerLoc.y,playerLoc.z,1000.0f,true);
                    set_status_text("PED CLEARED");
                }
                    break;
                case 1:
                {
                    GAMEPLAY::CLEAR_AREA_OF_VEHICLES(playerLoc.x,playerLoc.y,playerLoc.z,1000.0f,true,true,true,true,true);
                    set_status_text("VEHICLES CLEARED");
                }
                    break;
                default:
        ...
        {
            menu_beep();
            if (activeLineIndexClear == 0)
                activeLineIndexClear = lineCount;
            activeLineIndexClear--;
            waitTime = 150;
         } else
         if (bDown)
         {
            menu_beep();
            activeLineIndexClear++;
            if (activeLineIndexClear == lineCount)
                activeLineIndexClear = 0;
            waitTime = 150;
         }
    }
...
void process_spawnped_menu(){
    const float lineWidth = 250.0;
    const int lineCount = 3;
    
    std::string caption = "PED  OPTIONS";
    ...
    static struct {
        ...
        {"Spawn Kid", NULL,NULL},
        {"Spawn Adult", NULL, NULL},
        {"Spawn Senior", NULL, NULL}
    };
    ...
    while (true)
    {
        ...
        do
        {
            ...
            for (int i = 0; i < lineCount; i++)
                if (i != activeLineIndexSpawn)
                    draw_menu_line(line_as_str(lines[i].text, lines[i].pState),
                                   lineWidth, 9.0, 60.0 + i * 36.0, 0.0, 9.0, false, false);
            draw_menu_line(line_as_str(lines[activeLineIndexSpawn].text, lines[activeLineIndexSpawn].pState),
                           lineWidth + 1.0, 11.0, 56.0 + activeLineIndexSpawn * 36.0, 0.0, 7.0, true, false);
        ...
        if (bSelect){
            ...
            Vector3 playerLoc = ENTITY::GET_ENTITY_COORDS(e,true);
            switch(activeLineIndexSpawn){
                case 0:
                {
                    PED::CREATE_RANDOM_PED(playerLoc.x - 5.0f, playerLoc.y -5.0f,playerLoc.z);
                    //PED::CREATE_PED(28,111281960,playerLoc.x - 5.0f, playerLoc.y -5.0f,playerLoc.z,0.0,false,true);
                    set_status_text("Kid Spawned");
                }
                    break;
                case 1:
                {
                    PED::CREATE_RANDOM_PED(playerLoc.x -5.0f, playerLoc.y -5.0f,playerLoc.z);
                    //PED::CREATE_PED(6,368603149,playerLoc.x - 5.0f, playerLoc.y -5.0f,playerLoc.z,0.0,false,true);
                    set_status_text("Adult Spawned");
                }
                    break;
                case 2:
                {
                    PED::CREATE_RANDOM_PED((playerLoc.x -5.0f), (playerLoc.y -5.0f),playerLoc.z);
                    //PED::CREATE_PED(3,-1686040670,playerLoc.x - 5.0f, playerLoc.y -5.0f,playerLoc.z,0.0,false,true);
                    set_status_text("Senior Spawned");
                }
                    break;
                default:
                    if (lines[activeLineIndexSpawn].pState)
                        *lines[activeLineIndexSpawn].pState = !(*lines[activeLineIndexSpawn].pState);
                    if (lines[activeLineIndexSpawn].pUpdated)
                        *lines[activeLineIndexSpawn].pUpdated = true;
            }
        }else
        if (bBack || trainer_switch_pressed())
        {
...
void process_main_menu()
{
	...
	while (true)
	{
		// timed menu draw, used for pause after active line bswitch
		DWORD maxTickCount = GetTickCount() + waitTime;
		do 
		{
			// draw menu
